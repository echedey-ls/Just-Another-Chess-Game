...
class Menu_opciones_prejuego {
	...
	std::function<void(bool)> callback_btns = nullptr;
public:
	Menu_opciones_prejuego(std::function<void(bool)> callback_btns_) {
		btn_blancas.register_on_click(std::bind(callback_btns_, true));
		btn_negras.register_on_click(std::bind(callback_btns_, false));
	}
